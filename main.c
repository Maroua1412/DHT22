  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  //HAL_TIM_Base_Start(&htim2);
	  	  HAL_Delay(1000);
	  	  Data_Output(GPIOA, GPIO_PIN_4); //info vers le capteur
	  	  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET); // remmettre à l'etat bas
	  	  DWT_Delay_us(1200); //signal de commande > 1ms
	  	  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);// état haut
	  	  DWT_Delay_us(30); //signal de commande
	  	  Data_Input(GPIOA, GPIO_PIN_4); //info vers le microcontroleur
	  	 ...
	  	 if(check == (SUM))
	  	  ...
	  	  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET); //pour la prochaine lecture
